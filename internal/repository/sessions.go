...
func (r *SessionRepository) ListTemplates(ctx context.Context, batchID, teacherID string, offset, limit int) ([]models.SessionTemplate, int, error) {
	where := "WHERE 1=1"
	args := []any{}
	if batchID != "" {
		args = append(args, batchID)
		where += fmt.Sprintf(` AND st.batch_id = $%d`, len(args))
	}
	if teacherID != "" {
		args = append(args, teacherID)
		where += fmt.Sprintf(` AND st.teacher_id = $%d`, len(args))
	}
	...
	countQuery := `
		...
		JOIN batches b ON b.id = st.batch_id ` + where
	...
	query := fmt.Sprintf(`
		...
		%s ORDER BY st.day_of_week, st.start_time LIMIT $%d OFFSET $%d
...
func (r *SessionRepository) GetTemplate(ctx context.Context, id string) (*models.SessionTemplate, error) {
	var t models.SessionTemplate
	err := r.db.QueryRowContext(ctx, `
		SELECT st.id, st.batch_id, st.teacher_id, u.name, b.name, st.day_of_week,
		       st.start_time::text, st.end_time::text, st.created_at, st.updated_at
		...
		JOIN batches b ON b.id = st.batch_id
		WHERE st.id = $1
	`, id).Scan(&t.ID, &t.BatchID, &t.TeacherID, &t.TeacherName, &t.BatchName,
		&t.DayOfWeek, &t.StartTime, &t.EndTime, &t.CreatedAt, &t.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, httpx.NotFound("session template not found")
	}
	return &t, err
}

func (r *SessionRepository) UpdateTemplate(ctx context.Context, id string, dayOfWeek *int, startTime, endTime, teacherID string) (*models.SessionTemplate, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		...
		WHERE id = $1
		RETURNING id, batch_id, teacher_id, day_of_week, start_time::text, end_time::text, created_at, updated_at
	`, id, day, nullIfEmpty(startTime), nullIfEmpty(endTime), teacherID).Scan(
		&t.ID, &t.BatchID, &t.TeacherID, &t.DayOfWeek, &t.StartTime, &t.EndTime, &t.CreatedAt, &t.UpdatedAt,
	)
	if err == sql.ErrNoRows {
		return nil, httpx.NotFound("session template not found")
...
func (r *SessionRepository) DeleteTemplate(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx, `DELETE FROM session_templates WHERE id = $1`, id)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return httpx.NotFound("session template not found")
	}
	return nil
}
...
func (r *SessionRepository) GenerateSessions(ctx context.Context, startDate, endDate string, batchID *string) (int, error) {
	...
	if err != nil {
		return 0, httpx.InvalidInput("start_date must be YYYY-MM-DD")
	}
	end, err := time.Parse("2006-01-02", endDate)
	if err != nil {
		return 0, httpx.InvalidInput("end_date must be YYYY-MM-DD")
	}
...
func (r *SessionRepository) ListToday(ctx context.Context, teacherID string, offset, limit int) ([]models.Session, int, error) {
	where := ` WHERE s.session_date = CURRENT_DATE AND s.status = 'scheduled'`
	args := []any{}
	if teacherID != "" {
		args = append(args, teacherID)
		where += ` AND b.teacher_id = $1`
	}
	...
	countQuery := `
		...
		JOIN subjects sub ON sub.id = so.subject_id` + where
	if err := r.db.QueryRowContext(ctx, countQuery, args...).Scan(&total); err != nil {
		return nil, 0, err
	}

	args = append(args, limit, offset)
	query := sessionSelect + where + fmt.Sprintf(` ORDER BY s.start_time LIMIT $%d OFFSET $%d`, len(args)-1, len(args))
